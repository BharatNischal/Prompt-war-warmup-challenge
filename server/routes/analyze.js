  ...
  try {
    ...
    if (req.files?.fieldImages && req.files.fieldImages.length > 0) {
      for (let i = 0; i < req.files.fieldImages.length; i++) {
        ...
        uploadFieldImage(compressed.buffer, analysisId, i).catch((err) => {
          logger.warn('Image upload to GCS failed (non-critical)', { error: err.message }, requestId);
        });
      }
    }
    ...
    if (data.latitude !== undefined && data.longitude !== undefined) {
      try {
        ...
        logger.warn('Weather fetch failed, continuing without', { error: err.message }, requestId);
